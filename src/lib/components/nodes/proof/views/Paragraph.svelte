			...
			if (!hasChildren) {
				if (dispatch) {
					...
					if (preserved === new_line && new_line === '') {
						...
						rebuild_selection_head += 0; // Enter paragraph then line
						tr.doc.check();
