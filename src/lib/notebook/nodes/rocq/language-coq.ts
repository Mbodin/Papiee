...
function tokenBase(stream: StringStream, state: RocqCMState) {
	...
	*/
	...
	if (/\w/.test(ch)) stream.eatWhile(/[\w']/);
...
function tokenComment(stream: StringStream, state: RocqCMState) {
	...
	if (!state.commentLevel) state.tokenize = tokenBase;
